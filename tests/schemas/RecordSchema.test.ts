...
describe('.max(length)', () => {
  ...
  describe('when `length` is zero', () => {
    ...
    const schema = RecordSchema.create(NumberSchema.create()).max(0)
  ...
  describe('when `length` is a positive number', () => {
    ...
    const schema = RecordSchema.create(NumberSchema.create()).max(5)
...
describe('.some(callback)', () => {
  ...
  const schema = RecordSchema.create(NumberSchema.create()).some((value) => (value % 10 === 0))
...
describe('.every(callback)', () => {
  ...
  const schema = RecordSchema.create(NumberSchema.create()).every((value) => (value % 10 === 0))
